（関数 （） {
  ...
  constの resultDivided  =  ドキュメント。getElementById （'result-area' ）;
  constの tweetDivided  =  ドキュメント。getElementById （'tweet-area' ）;
  ...
  * /
  ...
  assessmentButton 。onclick  =  （） =>  {
      constの userNameの =  userNameInput 。値;
      ...
      const  パラグラフ =  ドキュメント。createElement （'p' ）;
      ...
      constの アンカー =  ドキュメント。createElement （'a' ）;
  ...
  * /
  機能 評価（userName ） {
      ...
      聞かせて sumOfcharCode  =  0を。
      用 （せ iは =  0 ;  I  <  userNameに。長さ;  I ++ ） {
      ...
      result  =  result 。置換（/{ userName } / g 、 userName ）;
       結果を返す;
  }
